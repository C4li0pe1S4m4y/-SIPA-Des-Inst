...
namespace CapaAD
{
    public class PlanAnualAD
    {
        ...
        public DataSet InformacionPac(int idPac)
        {
            ...
            {
                query = String.Format("CALL sp_slctPacListado('', {0}, 6);", idPac);
                conectar.AbrirConexion();
                consulta = new MySqlDataAdapter(query, conectar.conectar);
                consulta.Fill(tablaEnc);

                query = String.Format("CALL sp_slctPacListado('', {0}, 7);", idPac);
                conectar.AbrirConexion();
                consulta = new MySqlDataAdapter(query, conectar.conectar);
                consulta.Fill(tablaDet);
            }
        ...
        public DataSet InformacionPacDep(int idPac)
        {
            ...
            string query = String.Format("CALL sp_slctPacListado('', {0}, 5);", idPac);
            conectar.AbrirConexion();
            MySqlDataAdapter consulta = new MySqlDataAdapter(query, conectar.conectar);
            consulta.Fill(tablaEnc);
        ...
        public DataTable InformacionRenglonAccion(int idDetalleAccion,int anio)
        {
            conectar = new ConexionBD();
            DataTable tabla = new DataTable();
            string query = String.Format("CALL sp_slctPptoPac({0}, {1},{2});", idDetalleAccion, 2,anio);
            conectar.AbrirConexion();
            MySqlDataAdapter consulta = new MySqlDataAdapter(query, conectar.conectar);
            consulta.Fill(tabla);
            conectar.CerrarConexion();
