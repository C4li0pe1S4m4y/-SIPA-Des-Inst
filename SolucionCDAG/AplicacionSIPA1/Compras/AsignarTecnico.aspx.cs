    ...
    public partial class AsignarTecnico : System.Web.UI.Page
    {
        ...
        public void NuevaAprobacion()
        {
            try
            {
                ...
                pInsumoLN.DdlTecnicosCompras(ddlTecnicos);
                filtrarDvPedidos();
                filtrarGridDetalles();
                filtrarGridPpto();
        ...
        protected void filtrarDvPedidos()
        {
            try
            {
                ...
                {
                    dsResultado = pInsumoLN.InformacionPedido(idEncabezado, idtipoDocto, 0, anio.ToString(), 10);
                    ...
                    if (dsResultado.Tables["BUSQUEDA"].Rows.Count > 0 && dsResultado.Tables["BUSQUEDA"].Rows[0]["ID"].ToString() != "")
                    {
                        ...
                        int idTecnico = 0;
                        int.TryParse(dsResultado.Tables["BUSQUEDA"].Rows[0]["ID_TECNICO"].ToString(), out idTecnico);

                        btnAprobar.Visible = btnRechazar.Visible = true;
                        if (idTecnico > 0)
                        {
                            string tecnico = dsResultado.Tables["BUSQUEDA"].Rows[0]["TECNICO"].ToString();
                            lblError.Text = "El documento seleccionado ya fue asignado a: " + tecnico;
                            btnAprobar.Visible = btnRechazar.Visible = false;

                            //AGREGADO PARA CONTROLAR LAS REASIGNACIONES DE PEDIDOS A TÉCNICOS
                            pInsumoLN = new PedidosLN();
                            dsResultado = pInsumoLN.InformacionPermisos(0, 0, " AND a.usuario = ''" + Session["usuario"].ToString() + "'' AND c.id_tipo = 49 ", 12);
        ...
        protected void filtrarGridDetalles()
        {
            try
            {
                ...
                pInsumoLN = new PedidosLN();
                DataSet dsResultado = pInsumoLN.DetallesPedidoAprobacion(idSalida, idTipoSalida, "", 1,ddlAnios.SelectedValue);
        ...
        protected void btnAprobar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int.TryParse(rblTipoDocto.SelectedValue, out idTipoSalida);
                int.TryParse(ddlTecnicos.SelectedValue, out idTecnico);
        ...
        protected void btnRechazar_Click(object sender, EventArgs e)
        {
            ...
            //        NuevaAprobacion();
            //        lblSuccess.Text = "Solicitud No. " + idSalida + " RECHAZADA con éxito!";
