...
{
    public partial class PacVoBo2 : System.Web.UI.Page
    {
        ...
        public void NuevoEncabezadoPoa()
        {
            try
            {
                ...
                uUsuarios = new UsuariosLN();
                uUsuarios.dropUnidad(ddlUnidades);
        ...
        protected bool validarPoaIngresoPac(int idUnidad, int anio)
        {
            ...
            lblIdPoa.Text = "0";
            btnLimpiarC.Visible = btnGuardar.Visible = btnAprobar.Visible = btnRechazar.Visible = gridPac.Columns[1].Visible = false;
            try
            {                
                pOperativoLN = new PlanOperativoLN();                
                ...
                {
                    btnGuardar.Visible = btnAprobar.Visible = btnRechazar.Visible = gridPac.Columns[1].Visible = false;
                    lblErrorPac.Text = lblError.Text = "El CUADRO DE MANDO INTEGRAL seleccionado se encuenta en estado: " + estadoPoa;
                }
                else
                {
                    //btnGuardar.Visible = btnAprobar.Visible = btnRechazar.Visible = gridPac.Columns[1].Visible = true;
                    ...
                    if (!estadoPac.Split('-')[0].Trim().Equals("4"))
                    {
                        btnGuardar.Visible = btnAprobar.Visible = btnRechazar.Visible = gridPac.Columns[1].Visible = false;
        ...
        protected bool validarPoaListadoPac(int idUnidad, int anio)
        {
            ...
            try
            {
                ...
                {
                    lblCErrorPac.Text = lblCError.Text = "El PLAN ANUAL DE COMPRAS seleccionado se encuenta en estado: " + lblEstadoPac.Text + ". No se puede APROBAR o RECHAZAR";
                ...
                //4	Revisión Analista Compras
                else if (estadoPac.Equals("4"))
                {
                ...
                //2	Revisión Subgerencia
                else if (estadoPac.Equals("2"))
                {
                    lblCErrorPac.Text = lblCError.Text = "El PLAN ANUAL DE COMPRAS seleccionado se encuenta en estado: " + lblEstadoPac.Text + ". No se puede APROBAR o RECHAZAR";
                ...
                estadoPoa = dsPoa.Tables[0].Rows[0]["ID_ESTADO"].ToString();

                if (!estadoPoa.Equals("9"))
                {
                    lblCErrorPac.Text = lblCError.Text = "El CUADRO DE MANDO INTEGRAL no ha sido aprobado por DGE. No se puede APROBAR o RECHAZAR el PLAN ANUAL DE COMPRAS";
                ...
                else
                {
                    ...
                    if (!estadoPac.Split('-')[0].Trim().Equals("1") && !estadoPac.Split('-')[0].Trim().Equals("3") && !estadoPac.Split('-')[0].Trim().Equals("5"))
                    {
                        btnGuardar.Visible = btnEnviar.Visible = gridPac.Columns[1].Visible = false;
        ...
        protected void nuevaBusqueda()
        {
            try
            {
                ...
                gridPac.Columns[1].Visible = btnGuardar.Visible = btnLimpiarC.Visible = false;
                ...
                uUsuarios = new UsuariosLN();
                uUsuarios.dropUnidad(ddlCUnidades);
        ...
        protected void filtrarGrid()
        {
            try
            {
                ...
                pAnualLN = new PlanAnualLN();
                pAnualLN.GridListadoPacs(gridPac, idPoa);
        ...
        protected void btnAprobar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                txtObser.Text = string.Empty;
                cambiarEstado(6, "APROBADA");
            }
            catch (Exception ex)
            {
                lblCError.Text = "btnAprobar(). " + ex.Message;
            }             
        }
        ...
        protected void btnRechazar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtObser.Text.Equals(string.Empty))
                    lblCError.Text = "Llene el campo de observaciones.";
                else
                    cambiarEstado(5, "RECHAZADA");
            }
            catch (Exception ex)
            {
                lblCError.Text = "btnAprobar(). " + ex.Message;
        ...
        protected void cambiarEstado(int idEstado, string nombreEstado)
        {
            try
            {
                ...
                DataSet dsResultado = pAnualLN.ActualizarEstadoPac(idPoa, idEstado, anio, null, "", usuario, observaciones);

                if (bool.Parse(dsResultado.Tables[0].Rows[0]["ERRORES"].ToString()))
                    throw new Exception("No se INSERTÓ/ACTUALIZÓ el Plan Anual de Compras: " + dsResultado.Tables[0].Rows[0]["MSG_ERROR"].ToString());

                lblCSuccess.Text = "Planificación " + nombreEstado + " exitosamente!";
                btnAprobar.Visible = btnRechazar.Visible = false;
            }
            catch (Exception ex)
            {
                lblCError.Text = "cambiarEstado(). " + ex.Message;
            }
        }
        ...
        protected void generarReporte()
        {
            try
            {
                ...
                //if (idPoa > 0)
                {
                    ...
                    if(idPoa == 0)
                        dsResultado = reportes.ReportesSipa(0, 0, "PAC", 1);
                    else
                        dsResultado = reportes.ReportesSipa(idPoa, 0, "PAC", 2);
                    ...
                    object obj = gridPac.DataSource;
                    System.Data.DataTable tbl = dsResultado.Tables[1] as System.Data.DataTable;
                    System.Data.DataView dv = tbl.DefaultView;
