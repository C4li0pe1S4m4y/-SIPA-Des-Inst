...
namespace AplicacionSIPA1.Operativa
{
    public partial class VoBoN3 : System.Web.UI.Page
    {
        ...
        protected void filtrarGridPlan()
        {
            ...
            {
                if (ex.Message != "Cannot find column [anio].")
                {
        ...
        protected void cambiarEstado(int idEstado, string nombreEstado)
        {
            try
            {
                ...
                DataSet dsResultado = planOperativoLN.ActualizarEstadoPoa(int.Parse(lblIdPoa.Text), idEstado, anio, null, "", usuario, observaciones, ip[0], ip[1], ip[2], "Aprobacion N3", "Aprobar");
        ...
        protected void ddlDepend_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (idUnidad == 0)
                {
                    ...
                    planAccionLN.DdlDependenciasUsuario(ddlDependencias, Session["usuario"].ToString(), int.Parse(ddlUnidades.SelectedValue));
                    if (anio > 0 && idUnidad >= 0)
                        validarPoa(Convert.ToInt32(ddlUnidades.SelectedValue), anio);
                }
