        ...
        protected bool validarEstadoSeguimiento(int idSeguimiento)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    //EL SEGUIMIENTO ESTÁ EN ESTADO 7 Revisión Dirección
                    if (idEstadoSeguimiento == 7)
                    {
                        btnAprobar.Visible = btnRechazar.Visible = /*gridDet.Columns[0].Visible = gridDet.Columns[1].Visible =*/ true;
                        lblErrorPoa.Text = lblError.Text = "";
                        seguimientoValido = true;
            ...
            try
            {
                ...
                //if (validarControlesABC())
                {
                    if (validarEstadoSeguimiento(idSeguimiento))
                    {
                        ...
                        dsResultado = sSeguimientoLN.AprobadoN3(idSeguimiento, "", Session["usuario"].ToString());
            ...
            try
            {
                ...
                if (validarControlesABC())
                {
                    if (validarEstadoSeguimiento(idSeguimiento))
                    {
                        ...
                        dsResultado = sSeguimientoLN.RechazadoN3(idSeguimiento, txtObser.Text, Session["usuario"].ToString());
        ...
        protected void gridDet_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (validarControlesABCDetalle(gridDet.SelectedIndex))
                {
                    ...
                    string sAvanceKpi = gridDet.SelectedDataKey["AVANCE_KPI"].ToString(); //(gridDet.Rows[gridDet.SelectedIndex].FindControl("txtAvanceKpi") as TextBox).Text;
                    string sDescripcionAvanceKpi = gridDet.SelectedDataKey["DESCRIPCION_AVANCE_KPI"].ToString(); //(gridDet.Rows[gridDet.SelectedIndex].FindControl("txtDescripcionAvanceKpi") as TextBox).Text;

                    string sObservacionesDge = (gridDet.Rows[gridDet.SelectedIndex].FindControl("txtObservacionesDge") as TextBox).Text;
                    string sPlanAccion = "";

                    if ((gridDet.Rows[gridDet.SelectedIndex].FindControl("CheckBox1") as CheckBox).Checked == true)
                        sPlanAccion = "1";
                    else
                        sPlanAccion = "0";
