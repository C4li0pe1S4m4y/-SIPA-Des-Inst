...
{
    public partial class SeguimientoIngreso : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                if (dsResultado.Tables[1].Rows.Count > 0)
                {
                    ...
                    if (idSeguimiento > 0)
                    {
                        ...
                        int idEstado = 0;
                        int.TryParse(dsResultado.Tables["BUSQUEDA"].Rows[0]["ID_ESTADO"].ToString(), out idEstado);

                        //generarReporte(idSeguimiento);
                        //9 - APROBADO DIRECCIÓN
                        if (idEstado == 4 || idEstado == 7 || idEstado == 9)
                        {
                            generarReporte(idSeguimiento);
                        ...
                        {
                            lblError.Text = "No se puede imprimir el Seguimiento en el estado Actual";
        ...
        private bool validarControlesABC()
        {
            ...
            try
            {
                ...
                if (ddlMeses.SelectedValue.Equals("0") || ddlMeses.Items.Count == 0)
                {
                ...
                for (int i = 0; i < gridDet.Rows.Count; i++)
                {
                    ...
                    avanceKpi = (gridDet.Rows[i].FindControl("txtAvanceKpi") as TextBox).Text;
                    avanceKpi = avanceKpi.Replace('\'', ' ').Replace(';', ' ');
                    avanceKpi = avanceKpi.Trim();
                    (gridDet.Rows[i].FindControl("txtAvanceKpi") as TextBox).Text = avanceKpi;

                    if (avanceKpi.Equals(""))
                    {
                        (gridDet.Rows[i].FindControl("lblErrorAvanceKpi") as Label).Text = "Ingrese avance del KPI. ";

                        if (lblError.Text.Contains("Error en los detalles del seguimiento de CMI. ") == false)
                            lblError.Text += "Error en los detalles del seguimiento de CMI. ";
                    }

                    descripcionAvanceKpi = (gridDet.Rows[i].FindControl("txtDescripcionAvanceKpi") as TextBox).Text;
                    descripcionAvanceKpi = descripcionAvanceKpi.Replace('\'', ' ').Replace(';', ' ');
                    descripcionAvanceKpi = descripcionAvanceKpi.Trim();
                    (gridDet.Rows[i].FindControl("txtDescripcionAvanceKpi") as TextBox).Text = avanceKpi;

                    if (descripcionAvanceKpi.Equals(""))
                    {
                        (gridDet.Rows[i].FindControl("lblErrorDescripcionAvanceKpi") as Label).Text = "Ingrese descripción de avance del KPI. ";

                        if (lblError.Text.Contains("Error en los detalles del seguimiento de CMI. ") == false)
                            lblError.Text += "Error en los detalles del seguimiento de CMI. ";
                    }

                    avance = (gridDet.Rows[i].FindControl("txtAvance") as TextBox).Text;
                    avance = avance.Replace('\'', ' ').Replace(';', ' ');
                    avance = avance.Trim();
                    (gridDet.Rows[i].FindControl("txtAvance") as TextBox).Text = avance;

                    if (avance.Equals(""))
                    {
                        (gridDet.Rows[i].FindControl("lblErrorAvance") as Label).Text = "Ingrese avance correspondiente. ";
                        
                        if(lblError.Text.Contains("Error en los detalles del seguimiento de CMI. ") == false)
                            lblError.Text += "Error en los detalles del seguimiento de CMI. ";
                    }

                    mediosVerificacion = (gridDet.Rows[i].FindControl("txtMediosVerificacion") as TextBox).Text;
                    mediosVerificacion = mediosVerificacion.Replace('\'', ' ').Replace(';', ' ');
                    mediosVerificacion = mediosVerificacion.Trim();
                    (gridDet.Rows[i].FindControl("txtMediosVerificacion") as TextBox).Text = avance;

                    if (avance.Equals(""))
                    {
                        (gridDet.Rows[i].FindControl("lblErrorMediosVerificacion") as Label).Text = "Ingrese medios de verificación. ";
        ...
        protected bool validarPoaIngresoSeguimiento(int idUnidad, int anio)
        {
            ...
            try
            {
                ...
                gridDet.Columns[(gridDet.Columns.Count - 1)].Visible = false;
                lblIdPoa.Text = "0";
                ...
                //return true;
                if (!estadoPoa.Split('-')[0].Trim().Equals("9"))//(false)//
                {
                    //btnAnular.Visible = btnEnviar.Visible = btnGuardar.Visible = btnLimpiarC.Visible = gridDet.Columns[0].Visible = gridDet.Columns[1].Visible = false;
                    btnAnular.Visible = btnEnviar.Visible = gridDet.Columns[(gridDet.Columns.Count - 1)].Visible = btnLimpiarC.Visible = false;
                    lblErrorPoa.Text = lblError.Text = "El CUADRO DE MANDO INTEGRAL seleccionado se encuenta en estado: " + estadoPoa;
                }
                else
                {
                    //btnAnular.Visible = btnEnviar.Visible = btnGuardar.Visible = btnLimpiarC.Visible = gridDet.Columns[0].Visible = gridDet.Columns[1].Visible = true;
                    btnAnular.Visible = btnEnviar.Visible = gridDet.Columns[(gridDet.Columns.Count - 1)].Visible = btnLimpiarC.Visible = true;
                    poaValido = true;
                    lblErrorPoa.Text = string.Empty;
                }
            }
            ...
            try
            {
                ...
                if (idSeguimiento == 0)
                {
                    if (lblErrorPoa.Text.Equals("") || lblErrorPoa.Equals(string.Empty))
                    {
                        ...
                        btnAnular.Visible = btnEnviar.Visible = gridDet.Columns[(gridDet.Columns.Count - 1)].Visible = btnLimpiarC.Visible = /*gridDet.Columns[0].Visible = gridDet.Columns[1].Visible =*/ true;
                ...
                {
                    btnAnular.Visible = btnEnviar.Visible = gridDet.Columns[(gridDet.Columns.Count - 1)].Visible = btnLimpiarC.Visible = /*gridDet.Columns[0].Visible = gridDet.Columns[1].Visible =*/ false;
                    ...
                    //EL SEGUIMIENTO ESTÁ EN ESTADO INGRESADO, AL NO SER ENVIADO A REVISIÓN SE PUEDE MODIFICAR
                    if (idEstadoSeguimiento == 1)
                    {
                        btnAnular.Visible = btnEnviar.Visible = gridDet.Columns[(gridDet.Columns.Count - 1)].Visible = btnLimpiarC.Visible = /*gridDet.Columns[0].Visible = gridDet.Columns[1].Visible =*/ true;
                        lblErrorPoa.Text = lblError.Text = "";
                        seguimientoValido = true;
                    ...
                    }//EL SEGUIMIENTO ESTÁ EN ESTADO 2	Revisión Subgerencia, 4	Revisión Analista POA, 7, Revisión Dirección, 9	Aprobado Dirección Y NO SE PUEDE MODIFICAR
                    else if (idEstadoSeguimiento == 2 || idEstadoSeguimiento == 4 || idEstadoSeguimiento == 7 || idEstadoSeguimiento == 9)
                    {
                        btnAnular.Visible = btnEnviar.Visible = gridDet.Columns[(gridDet.Columns.Count - 1)].Visible = btnLimpiarC.Visible = /*ridDet.Columns[0].Visible = gridDet.Columns[1].Visible =*/ false;
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                if (validarControlesABC())
                {
                    ...
                    for (int i = 0; i < gridDet.Rows.Count; i++)
                    {
                        int.TryParse(gridDet.DataKeys[i].Values["ID_SEGUIMIENTO_CMI_DET"].ToString(), out idSeguimientoCmiDet);
                        int.TryParse(gridDet.DataKeys[i].Values["ID_ACCION"].ToString(), out idAccion);

                        string sPptoAnual = gridDet.DataKeys[i].Values["PPTO_ANUAL"].ToString();
                        string sAvancePptoCuatrimestral = gridDet.DataKeys[i].Values["AVANCE_PPTO_CUATRIMESTRAL"].ToString();
                        string sAvancePptoAcumulado = gridDet.DataKeys[i].Values["AVANCE_PPTO_ACUMULADO"].ToString();
                        string sSaldo = gridDet.DataKeys[i].Values["SALDO"].ToString();
                        ...
                        dr["DESCRIPCION"] = (gridDet.Rows[i].FindControl("txtAvance") as TextBox).Text;
                        ...
                        dr["MEDIOS_VERIFICACION"] = (gridDet.Rows[i].FindControl("txtMediosVerificacion") as TextBox).Text;
                        dr["AVANCE_KPI"] = "null";
                        dr["ANEXO"] = "";
                        dr["OBSERVACIONES_DGE"] = "";
                        dr["PLAN_ACCION"] = "0";
                        dr["ACTIVO"] = "1";
                    ...
                    if (validarEstadoSeguimiento(idSeguimientoCmi))
                    {
                        ...
                        NuevoSeguimientoDet();
                        lblSuccess.Text = "Informe de seguimiento al CMI No. " + idSeguimientoCmi.ToString() +" ALMACENADO/MODIFICADO exitosamente: ";
                    }
                }
            }
            catch (Exception ex)
        ...
        protected void btnEnviar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (validarControlesABC())
                {
                    ...
                    {
                        if (idSeguimiento == 0)
                            throw new Exception("No existe Informe para finalizar");

                        sSeguimientoLN = new SeguimientoLN();
                        DataSet dsResultado = new DataSet();

                        dsResultado = sSeguimientoLN.EnviarSeguimientoARevision(idSeguimiento, "", Session["usuario"].ToString());
            ...
            try
            {
                ...
                if (idSeguimiento > 0)
                {
                    ...
                    /*if (idEstado == 9)
                        ...
                        lblError.Text = "Sólo se podrá imprimir cuando el informe esté en estado 9 - Aprobado Dirección";*/

                }
            }
            catch (Exception ex)
            {
                throw new Exception("btnImprimir(). " + ex.Message);
            }
        }
        ...
        protected void generarReporte(int idEncabezado)
        {
            ...
            try
            {
                if (idEncabezado > 0)
                {
                    ...
                    sSeguimientoLN = new SeguimientoLN();
                    DataSet dsResultado = sSeguimientoLN.InformacionSeguimientos(0, 0, " AND a.id_seguimiento_cmi = " + idEncabezado.ToString(), 2);

                    if (bool.Parse(dsResultado.Tables[0].Rows[0]["ERRORES"].ToString()))
                        throw new Exception("No se CONSULTÓ la información del informe de seguimiento (encabezado): " + dsResultado.Tables[0].Rows[0]["MSG_ERROR"].ToString());
                    ...
                    int idPoa, anio = 0;
                    int.TryParse(lblIdPoa.Text, out idPoa);
                    int.TryParse(ddlMeses.SelectedValue, out anio);

                    dsResultado = sSeguimientoLN.InformacionSeguimientos(idPoa, anio, "" + idEncabezado.ToString(), 1);

                    if (bool.Parse(dsResultado.Tables[0].Rows[0]["ERRORES"].ToString()))
                        throw new Exception("No se CONSULTÓ la información del informe de seguimiento (detalles): " + dsResultado.Tables[0].Rows[0]["MSG_ERROR"].ToString());
                    ...
                    string nombreReporte = "GES-FOR-03";

                    rViewer.LocalReport.ReportEmbeddedResource = "\\Reportes/rptGES_FOR_03.rdlc";
                    rViewer.LocalReport.ReportPath = @"Reportes\\rptGES_FOR_03.rdlc";   
                    ...
                    string jScript = "javascript:window.open('" + reDireccion + "','GES-FOR-03'," + "'directories=no, location=no, menubar=no, scrollbars=yes, statusbar=no, tittlebar=no, width=750, height=400');";
        ...
        protected void gridDet_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (validarControlesABCDetalle(gridDet.SelectedIndex))
                {
                    ...
                    if (validarEstadoSeguimiento(idSeguimientoCmi))
                    {
                        ...
                        if (idSeguimientoCmi == 0)
                        {
                            dsResultado = sSeguimientoLN.AlmacenarSeguimiento(sSeguimientoEN,Session["usuario"].ToString());
                        ...
                        string sDescripcion = (gridDet.Rows[gridDet.SelectedIndex].FindControl("txtAvance") as TextBox).Text;
                        string sMediosVerificacion = (gridDet.Rows[gridDet.SelectedIndex].FindControl("txtMediosVerificacion") as TextBox).Text;
                        ...
                        dsResultado = sSeguimientoLN.AlmacenarSeguimientoDet(dsDetalles, 1);
            ...
            catch (Exception ex)
            {
                ...
                ScriptManager.RegisterStartupScript(this, typeof(string), "Mensaje", "alert('gridDet(): " + ex.Message + "');", true);
            }
        }
        ...
        private bool validarControlesABCDetalle(int indexGrid)
        {
            ...
            //DE ESTA MANERA PERMITO QUE SE LLENEN SOLO LAS COLUMANAS DEL KPI Y DESPUÉS LAS DEL AVANCE DE LA META CUATRIMESTRAL
            //EN EL BOTÓN ENVIAR SI VALIDO QUE TODOS LOS CAMPOS ESTEN LLENOS
            return true;
            try
            {
                //AVANCE KPI
                ...
                //AVANCE META CUATRIMESTRAL
                Label lblErrorAvance = (gridDet.Rows[gridDet.SelectedIndex].FindControl("lblErrorAvance") as Label);
                string sAvance = (gridDet.Rows[gridDet.SelectedIndex].FindControl("txtAvance") as TextBox).Text;
                sAvance = sAvance.Replace('\'', ' ').Replace(';', ' ');
                sAvance = sAvance.Trim();
                (gridDet.Rows[gridDet.SelectedIndex].FindControl("txtAvance") as TextBox).Text = sAvance;

                if (sAvance.Equals(""))
                    lblErrorAvance.Text = "Ingrese un valor";
                else
                    lblErrorAvance.Text = string.Empty;

                //MEDIOS DE VERIFICACION AVANCE META CUATRIMESTRAL
                Label lblErrorMediosVerificacion = (gridDet.Rows[gridDet.SelectedIndex].FindControl("lblErrorMediosVerificacion") as Label);
                string sMediosVerificacion = (gridDet.Rows[gridDet.SelectedIndex].FindControl("txtMediosVerificacion") as TextBox).Text;
                sMediosVerificacion = sMediosVerificacion.Replace('\'', ' ').Replace(';', ' ');
                sMediosVerificacion = sMediosVerificacion.Trim();
                (gridDet.Rows[gridDet.SelectedIndex].FindControl("txtMediosVerificacion") as TextBox).Text = sAvance;

                if (sAvance.Equals(""))
                    lblErrorMediosVerificacion.Text = "Ingrese un valor";
                else
                    lblErrorMediosVerificacion.Text = string.Empty;

                if (lblErrorAvanceKpi.Text.Equals(string.Empty) && lblErrorDescripcionAvanceKpi.Text.Equals(string.Empty) && lblErrorAvance.Text.Equals(string.Empty) && lblErrorMediosVerificacion.Text.Equals(string.Empty))
                    controlesValidos = true;
