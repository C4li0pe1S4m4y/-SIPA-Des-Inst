...
{
    public partial class VoBoCoordinadorCompras : System.Web.UI.Page
    {
        ...
        protected void btnAprobar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (validarEstadoPedido(idPedido))
                {
                    ...
                    DataSet dsResultado = pInsumoLN.AprobacionCoordinadorCompras(idPedido, idTipoDocumento, observaciones, usuario, ip[0], ip[1], ip[2]);
                    ...
                    objEC.EnvioCorreo(planOperativoLN.ObtenerCorreoxUsuario(jefeTemp[1].Trim()), "Nueva Requiscion Aprobada por Centro de Costos", " Requisicion No.  " + idPedido + ", " + lblSuccess.Text, usuario);
                    objEC.EnvioCorreo(planOperativoLN.ObtenerCorreoxUsuario(solicitanteTemp[1].Trim()), "Nueva Requiscion Aprobada por Centro de Costos", " Requisicion No.  " + idPedido + ", " + lblSuccess.Text, usuario);
        ...
        protected void btnRechazar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    string[] solicitanteTemp = solicitandte.Split('-');
                    string[] jefeTemp = jefe.Split('-');
                    DataSet dsResultado = pInsumoLN.RechazoCoordinadorCompras(idPedido, idTipoDocumento, observaciones, usuario, ip[0], ip[1], ip[2]);
                    ...
                    EnvioDeCorreos objEC = new EnvioDeCorreos();
                    objEC.EnvioCorreo(planOperativoLN.ObtenerCorreoxUsuario(jefeTemp[1].Trim()), "Nueva REQUISICIÓN/VALE RECHAZADA por Centro de Costos", " Requisicion No.  " + idPedido + ", " + lblSuccess.Text, usuario);
                    objEC.EnvioCorreo(planOperativoLN.ObtenerCorreoxUsuario(solicitanteTemp[1].Trim()), "Nueva REQUISICIÓN/VALE RECHAZADA por Centro de Costos", " Requisicion No.  " + idPedido + ", " + lblSuccess.Text, usuario);
                    NuevaAprobacion();
